...
console.log(
  'Blackjack reference: Stake Originals rules; every step is one simulated native bet; no wallet transactions.',
);
...
console.log(`Expected bets per completed hand: ${decimal(evaluation.expectedAtomicBets)}.`);
...
for (const node of plan.nodes) {
  ...
  for (const action of node.actions) {
    console.log(
      `    ${action.id}: required ${amount(action.requiredCash)}; ${action.transition.kind === 'bet' ? `${action.transition.bet.prizes.length} prizes over ${action.transition.successors.length} successors` : action.transition.kind}`,
    );
  }
}
