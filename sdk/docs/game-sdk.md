...
`developer` is the address of the account you publish the game from. It earns the game's commission, and its key is the one your game's server settles its [bets that settle later](#bets-that-settle-later) with, if it has any: the casino lets only that key draw and settle the game's bets, and every bet that settles later signs it.
...
A bet on the player's own round settles in the one request that places it. A game with a server can also place bets that **settle later**, by the game's server: a roulette wheel many players ride together, a crash curve, a football match. The server is the game's **referee**, and it signs with your key: the key of the account you publish the game from, whose address the manifest names as its `developer`. So the server holds everything that account holds: your games, their commission and your bank, which pays the splits the key signs. Keep the key as safe as all of that, or publish the game from an account of its own.
...
**One Worker.** A game with a referee ships page and server as one Cloudflare Worker: `dist/` as static assets and a `server/worker.ts` that answers `/api/`, with a Durable Object for its state. Page and server share an origin, so the build's `connect-src 'self'` holds. The server takes `CASINO_URL` and `GAME_NAME` as variables and `DEVELOPER_KEY` as a secret (`wrangler secret put DEVELOPER_KEY`; locally, `.dev.vars`), the key's address and the name making the game's key. For local play, the casino's launcher runs such a game in [games/](../../games/) with `wrangler dev`, against its own casino and with the key of the local `@hookedin`, which publishes it at the address its `wrangler.jsonc` gives `wrangler dev`.
...
| Code                 | Meaning                                                                                                        |
| -------------------- | -------------------------------------------------------------------------------------------------------------- |
| `invalid-request`    | The request is malformed, its envelope `id` did not rise, or it is a `game.place` for a game published nowhere |
