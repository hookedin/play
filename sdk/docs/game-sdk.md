...
A game is its publisher and the name they publish it under: its key, `keccak256(abi.encode(publisher, name))`, stays the same whoever it pays and wherever it is served. Publish your game in the wallet: open **My games** and give the game a name (`[a-z0-9-]`, at most 32 characters) and its manifest URL. It is then at `@<your alias>/<game name>` — or `~<your uname>/<game name>` if you have no alias — for anyone, and in your library. A profile holds a hundred games, and publishing needs a funded ETH channel. Publishing records the `developer` the manifest names, the address that earns the game's commission, which need not be yours. The wallet refuses a published game whose manifest names another developer than the one it was published with, so a host that is taken over cannot redirect the commission; to pay another address, change the manifest's `developer` and publish the game again. The URL is only where the game is served: move hosts by publishing the same name at the new URL. Either way the game keeps its key, its bets and its players' receipts. The library a deployment ships with is what `@hookedin` publishes, from [catalog.json](../../catalog.json); to be in it, open an issue or pull request on [hookedin/play](https://github.com/hookedin/play). A manifest that is published nowhere can still be loaded directly: open Games, choose **Add a custom game** and paste the manifest URL. It then has the key of its manifest's developer and URL, so it is a different game from any you publish.
...
| `game.bet`          | `{id, stake, prizes, group?}`                                                  | Its receipt: `settled` or `rejected`                                                 |
| `game.place`        | `{id, stake, prizes, round, group?}` or `{id, stake, terms, deadline, group?}` | Its receipt at once: `placed` or `rejected`. See [below](#bets-that-settle-later)    |
...
`HookedIn` has a typed method for each: `receipt`, `bet`, `place`, `payment` and `requestFunds`; `HookedIn.call(method, params)` sends any of them.
...
Every reply about an operation is one receipt, whichever method asked, and every receipt a game gets is one the wallet checked. It is `{id, kind, status}` under your own `id`: `kind` is `bet` or `payment`, and `status` is `settled` (done, and what it paid is in the channel), `rejected` (declined with a signed checkpoint that leaves the balance unchanged), `placed` (a bet that settles later: its stake taken, the bet final) or `refunded` (nobody settled it by its deadline, and its stake is back in the channel). A settled bet's `basis` says what its payout rests on: `outcome`, the round's revealed secret and seed, which the wallet checked against the hashes the bet signed, or `referee`, its referee's signed split, which is the referee's word. With them come a bet's `stake`, `prizes` or `terms`, `round`, `deadline` and `group` as it was placed; a bet that settles later's `bet`, the hash that names it at the casino and to your referee; a settled bet's `outcome` (when it rode a round); `payout`, what a settled or refunded bet paid; and a rejection's or refund's `reason`. The signed evidence, the player's channel and its balance stay in the wallet.
...
| `game.receipt` | `{receipt}`          | A bet this game placed has settled or come back, and the wallet has collected it |

`HookedIn.onBalance(listener)` and `HookedIn.onReceipt(listener)` hear them; each returns a function that stops listening.

`wallet.hello` also carries `limits`: `prizes`, the most one bet holds; `outcomeSpace`, the size of the space a prize range lies in; `bets`, the most one round takes, and `cells`, the most distinct outcomes they may cut it into; `round`, how long a referee's round takes bets, and `deadline`, the furthest ahead a bet with terms may have its deadline, both in milliseconds; `terms`, the most a bet's terms take as canonical JSON; and `group`, the longest group label. They are part of the protocol revision, and the same numbers the casino reports in `GET /api/config` as `limits`, so a wallet and a casino that disagree stop before signing anything. Read them rather than carrying copies. A referee reads the same as its kit's `limits`.
...
A bet on the player's own round settles in the one request that places it. A game with a server can also place bets that **settle later**, by the game's server: a roulette wheel many players ride together, a crash curve, a football match. The server is the game's **referee**. A referee is just a key, the one you publish the game with as its manifest's `referee`. A referee that only draws needs no channel and holds no money; a split it signs is paid from its own bank, so keep the key as safe as the bank.

The page places such a bet with `HookedIn.place` (`game.place`). The wallet signs a debit and **sends it to the casino itself**: the reply is the bet's receipt, `placed`, with `bet`, the hash that names it; the stake has left the game's balance, and **the bet is final** — there is no taking it back. The referee never touches it. The same request again returns the receipt as it stands. Once the bet has settled, or come back at its deadline, the wallet checks what settled it, collects what it paid into the channel, raises the game's limit by it and pushes the new receipt to the page as a `game.receipt` event, which `HookedIn.onReceipt` hears. The wallet looks by itself every few seconds. A page that learns from its own server that its bet has settled can ask `game.receipt` with the bet's `id`: the reply is the receipt as it stands, and the wallet looks at once. After a reload, `game.receipt` finds the bet. The referee relays nothing the wallet relies on. Until the wallet collects it, what the bet pays or refunds is the casino's promise, outside the principal the contract protects, as the [trust model](../../README.md#trust-model) says.

```ts
import { createReferee, gameKey } from '@hookedin/play/sdk/referee';

// Your game's key: the address you publish it from, and the name you publish it under.
...
A published game's key is also in its profile entry. The kit checks `refereeProtocol` in the casino's `GET /api/config`, a hash of only what a referee shares with the casino (`RefereeAccess`, `Commit`, `Settlement`, the outcome and the limits), so a change to what only wallets sign does not stop it. Each open bet comes with its `bet` hash, `uname`, `asset`, `stake`, its `round` and `prizes` or its `terms`, and `placedAt`, the casino's time it took the bet.

**A bet with prizes** rides a round of your referee's, and is drawn against the bankroll. A round belongs to your game and one asset. `referee.open(asset)` is the round your players bet on: the casino names it, the hash of a secret it keeps, and the referee kit commits the seed it will draw it with, derived from your key and the round, before anybody bets on it. It is the same round until it is drawn or its deadline passes, so asking again is safe. A round takes bets for `limits.round`, ten minutes from when the casino names it: that is its `deadline`. Not drawn by then, its bets are refunded.

```ts
const round = await referee.open('eth'); // tell your pages round.id
```

The page gets the round's `id` from your server and bets with `HookedIn.place({id, stake, prizes, round})`; the bet takes its round's deadline. The wallet reads the round before it signs: it must be open, your game's, in the player's asset and committed by your referee. The bet names the round and the seed hash, so its outcome is fixed before it is placed, and neither you nor the casino knows it alone. A round that takes no more bets, drawn or past its deadline, is refused with `round-closed` before anything is signed. The casino takes each bet against the bankroll as it is placed, with every bet the round took before it: bets on the same side share the capacity one would have had, and bets on opposite sides hedge each other. It holds back the round's worst case until the round is drawn or its bets refunded. A bet that does not fit, reaches the casino after its round closed, or finds it holding `limits.bets` bets is declined with a signed rejection, its stake still the player's. When your game is ready — the wheel's betting time is up — your server draws the round on one 64-bit outcome:

```ts
const { outcome, bets } = await referee.draw(round.id);
```

The casino reveals the secret and pays every bet the round took what its prizes pay, and refunds none. Equal ranges pay together, disjoint ranges never both pay, and a player's several chips are simply their prizes; in a crash game with no cash-out by hand, each automatic cash-out is one prize. Price a layout before offering it with the casino's own rule ([`admits`](../src/admits.ts)). Save the round's `id` before you draw it: drawing it again, after a lost reply or a restart, is the same draw, and `referee.round(id)` reads any round as anyone may. A round with no open bets is not drawn. Once a round is drawn, `referee.open` names the next. You choose when a round is drawn, never what it pays: its outcome was fixed before any bet on it. [Roulette](../../games/roulette/) is the reference.

**A bet with terms** pays what your referee signs. The page bets with `HookedIn.place({id, stake, terms, deadline})`: `terms` is your game's own JSON object, up to 4,096 bytes, saying what the player bet on (a pick and its odds, a cash-out made while the round runs), and the player signs it; its numbers are whole, so write odds like `'2.1'` as strings. `deadline`, in unix milliseconds and within `limits.deadline`, is when the bet is refunded if nobody has settled it. A crash game with cash-out by hand settles every bet this way, with a crash point its referee keeps to itself: its word. A draw cannot do it: to know when to crash, the referee would have to draw at take-off, and a draw is public, so every page would know the crash point. Your server settles any open bet whenever it chooses:

```ts
await referee.settle(open.map(bet => ({ bet: bet.bet, player: cashedOut(bet), casino: share(bet) })));
```

`player` is what the player is paid and `casino` what the casino is given, each signed here with a `Settlement` over the bet's hash. What they come to beyond the stake is paid from your referee's own **bank** at the casino, which keeps the rest of the stake: the bank of the account at the referee's address. Import the referee's key in the wallet's **Settings**, and that account deposits into its bank, and takes money out, on **My games**. Nothing in it is reserved, and a batch the bank cannot pay is refused whole (`bank-short`). Whether you can pay what your referee settles is between you and your players, outside HookedIn: playing your game trusts you for its outcomes and its payments ([settled trade-offs](../../architecture.md#settled-trade-offs)). Settle each player's bet the moment it is decided — a cash-out, a final whistle — so they are paid at once. The casino asks for about half of what each bet is expected to earn you as its `casino` part; nothing enforces it. Refund a bet you should not have taken, a bet placed after your round began for one, by settling it with `player` equal to its stake. A page can prove to your server that a bet is its own by putting the hash of a secret it keeps in the terms. The result is your word, signed.

A bet with prizes is only ever drawn, and a bet with terms only ever split. A bet nobody settles by its deadline is refunded; a bet is in the asset the player plays with, and a player has at most 64 bets open in one asset. See [bets that settle later](../../docs/protocol.md#bets-that-settle-later).
...
**One Worker.** A game with a referee ships page and server as one Cloudflare Worker: `dist/` as static assets and a `server/worker.ts` that answers `/api/`, with a Durable Object for its state. Page and server share an origin, so the build's `connect-src 'self'` holds. The server takes `CASINO_URL`, `PUBLISHER` and `GAME_NAME` as variables, the last two making the game's key, and `REFEREE_KEY` as a secret (`wrangler secret put REFEREE_KEY`; locally, `.dev.vars`). For local play, the casino's launcher runs such a game in [games/](../../games/) with `wrangler dev`, against its own casino and with a fresh `REFEREE_KEY`, and publishes it in `@hookedin` at the address its `wrangler.jsonc` gives `wrangler dev`, with that key as its referee.
...
| Code                 | Meaning                                                                                                      |
| -------------------- | ------------------------------------------------------------------------------------------------------------ |
| `invalid-request`    | The request is malformed, its envelope `id` did not rise, or it is a `game.place` for a game with no referee |
...
| `round-closed`       | A placed bet names a round that takes no more bets: drawn, or past its deadline                              |
