...
The trust assumptions stated in [README.md](README.md#trust-model) and [architecture.md](architecture.md) are accepted design choices: one operator controls signing and the bankroll, winnings above the deposit are unsecured claims on the shared pool, the player must challenge a stale close within 24 hours, the casino can withhold completion, a game's developer and the casino together could know the outcome of the developer's round in advance and leave winning developer bets uncovered, a developer bet with terms pays what its developer signs, a developer bet is paid only if its developer pays, and bankroll fund shares are the casino's promise. Reports that restate these are welcome as design feedback in a public issue.
