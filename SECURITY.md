...
The trust assumptions stated in [the trust model](docs/overview/trust-model.md) and [the architecture](docs/overview/architecture.md) are accepted design choices: one operator controls signing and the bankroll, winnings above the deposit are unsecured claims on the shared pool, the player must challenge a stale close within 24 hours, the casino can withhold completion, a developer bet pays what its developer signs and whatever a game's own scheme promises beyond that is the game's, a developer bet is paid only if its developer pays, and bankroll fund shares are the casino's promise. Reports that restate these are welcome as design feedback in a public issue.
