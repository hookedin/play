...
The trust assumptions stated in [README.md](README.md#trust-model) and [architecture.md](architecture.md) are accepted design choices: one operator controls signing and the bankroll, winnings above the deposit are unsecured claims on the shared pool, the player must challenge a stale close within 24 hours, the casino can withhold completion, a host and the casino together could choose a hosted round's outcome, and bankroll fund shares and money on a table are the casino's promise. Reports that restate these are welcome as design feedback in a public issue.
