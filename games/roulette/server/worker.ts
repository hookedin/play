...
interface Env {
  ...
  CASINO_URL: string;
  /** The game as it is published: its publisher's address and the name they published it under, which make its key. */
  PUBLISHER: string;
  GAME_NAME: string;
      ...
      return new Wheel(
        {
          referee: await createReferee({
            ...
            game: gameKey({ publisher: this.env.PUBLISHER, name: this.env.GAME_NAME }),
          }),
          asset: assetOf(url),
          now: () => Date.now(),
