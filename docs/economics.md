...
Signed results change the player balance and casino accounting off-chain. Casino fees are retained in bankroll equity: losses add S-F/2 and wins subtract W+F/2 to reported betting bankroll. Admission uses the conservative full-fee inequality and reserves W+F. The full even fee F accrues equally to developer and casino earnings, once per completed bet. The service computes F from current capital excluding other in-flight reservations, reserves W+F before outcome computation, and rechecks capacity after asynchronous signing, reducing commission if necessary while the wager remains safe. It never changes the player's signed stake, net win or probability. Insufficient capacity produces a joint higher-sequence rejection checkpoint with unchanged balance, no consumed preimage and no commission. Reservations are per operation; they do not fund a whole future hand or stop an owner from withdrawing on-chain.
