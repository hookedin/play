...
Correlation cuts both ways. Prizes on the **same** range stack: two players on red are one double-sized wager, so a table shares the capacity a lone player would have had (`sum of same-side net wins / B <= edge`) instead of multiplying it. Prizes on **disjoint** ranges hedge: what red wins, black loses, and the table earns capacity no set of independent bets could. It makes no difference who holds them: one player with a chip on red and a chip on black is priced exactly like two players holding one each, and overlapping chips priced as they stand equal the same chips flattened to one payout per pocket. `vectors/bets.json` records one red, two reds, a red with a black, and one player's overlapping chips, at the same bankroll. A house pot fills one entry at a time, and each entry is assessed together with every entry already in it: the pot is admitted again as a whole with the newcomer, who is declined alone if it does not fit. So it matters who arrives first. A third red may be declined where a black is welcome, and the same red fits once the black is in. An entry that was admitted stays admitted. The pot reserves its worst cell from the first entry until it ends. Commission is decided once, when it is resolved, over the entries it ended with and against the capital there is then; it is operator accounting and never a reason to decline an entry already taken. A pot holds at most 256 entries and 128 distinct cells, which bounds the exact-integer products; a roulette table stays within the wheel's 37 pockets however many players sit down.
...
## Pots the bankroll does not back

Only a house pot is a wager against the bankroll, priced like a bet. A developer's pot is the developer's risk: their bank at the casino keeps what the pot does not pay and pays what it pays beyond its entries, and nothing of it is reserved, so the casino neither admits its entries by Kelly nor takes commission on them; `BANK_FEE_BASIS_POINTS`, what the casino would charge the bank on each pot, is 0. A players' pot moves money only between its players: its rake, capped by the pot, is the one commission, shared half to the developer and half to the casino like a bet's.

## Available capital and concurrency

Signed results change the player balance and casino accounting off-chain. Casino fees are retained in bankroll equity: losses add S-F/2 and wins subtract W+F/2 to reported betting bankroll. Admission uses the conservative full-fee inequality and reserves W+F. Each bet's even share of F accrues equally to its developer and casino earnings, once per completed bet. The service decides admission from current capital excluding other in-flight reservations and reserves the round's worst case before it reads the round's secret; a wager it has admitted settles. A bet on a channel's own round carries the commission its admission priced; a house pot's commission is priced when it is resolved, and only that gives way if capital has moved by then. It never changes the player's signed stake, net win or probability. Insufficient capacity produces a joint higher-sequence rejection checkpoint with unchanged balance and no commission. An entry declined from a house pot leaves the pot open for the entries it already has; a channel's own round is declined with its bet, revealed at once and settling nothing. Reservations are per round; they do not fund a whole future hand or stop an owner from withdrawing on-chain.
