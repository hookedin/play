...
Correlation cuts both ways. Prizes on the **same** range stack: two players on red are one double-sized wager, so a table shares the capacity a lone player would have had (`sum of same-side net wins / B <= edge`) instead of multiplying it. Prizes on **disjoint** ranges hedge: what red wins, black loses, and the table earns capacity no set of independent bets could. It makes no difference who holds them: one player with a chip on red and a chip on black is priced exactly like two players holding one each, and overlapping chips priced as they stand equal the same chips flattened to one payout per pocket. `vectors/bets.json` records one red, two reds, a red with a black, and one player's overlapping chips, at the same bankroll. A draw takes its round's bets one at a time, in the order the casino took them, and each is assessed together with every bet admitted before it: the draw is admitted again as a whole with the newcomer, who is refunded alone if it does not fit. So it matters who comes first. A third red may be refused where a black is welcome, and the same red fits once the black is in. The whole draw is decided before the round's secret is read, and settled in the same request, so it holds its worst cell only for as long as that takes. Commission is decided over the bets it admitted, against the capital there is then. A round holds at most 256 bets, and a draw 128 distinct cells, which bounds the exact-integer products; a roulette table stays within the wheel's 37 pockets however many players sit down.
