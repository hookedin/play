...
All monetary quantities are integer wei. The independent client selects a stake and the prizes it can pay. The casino admits the bet against its current unreserved bankroll and sets commission when the bet's round settles, against the bankroll there is then. The wallet and settlement contract verify the exact terms and balance arithmetic; commission is operator accounting and does not add a player debit. The receipt of a bet on a channel's own round reports its commission for display; the wallet does not independently verify that operator accounting.
...
Correlation cuts both ways. Prizes on the **same** range stack: two players on red are one double-sized wager, so a table shares the capacity a lone player would have had (`sum of same-side net wins / B <= edge`) instead of multiplying it. Prizes on **disjoint** ranges hedge: what red wins, black loses, and the table earns capacity no set of independent bets could. It makes no difference who holds them: one player with a chip on red and a chip on black is priced exactly like two players holding one each, and overlapping chips priced as they stand equal the same chips flattened to one payout per pocket. `vectors/bets.json` records one red, two reds, a red with a black, and one player's overlapping chips, at the same bankroll. A referee's round takes its bets one at a time, as they are placed, and each is assessed together with every bet the round took before it: the round is admitted again as a whole with the newcomer, who is declined alone if it does not fit. So it matters who comes first. A third red may be refused where a black is welcome, and the same red fits once the black is in. Every decision is made before anybody could know the outcome, and the round holds back its worst cell until it is drawn or its bets are refunded, so the draw pays every bet it took. Commission is priced over all of them at the draw, against the capital there is then; a bankroll that has fallen below what prices the round takes no commission from it. A round holds at most 256 bets and 128 distinct cells, which bounds the exact-integer products; a roulette table stays within the wheel's 37 pockets however many players sit down.
...
The developer is the account that publishes the game; a game nobody publishes has none, and all of its commission is the casino's. Each bet's rounding leaves under two wei in the bankroll. Probability rounding can likewise leave a slightly more favorable residual bet. JavaScript BigInt preserves the exact intermediate products.

This defines excess profit as the amount that can be removed while leaving the bankroll a Kelly-compliant residual wager. Charging only that excess is [a settled trade-off](../architecture.md#settled-trade-offs). A bet is admitted when the condition holds with `F = 0`, and the left side falls as `F` grows, so a fee fixed first would turn away bets the bankroll could take; setting `F` afterwards admits every one of them and charges only the surplus. Subtracting the two advertised edge percentages alone is not generally correct: doing so ignores how an outcome-independent commission changes the bankroll's exposure.
...
Signed results change the player balance and casino accounting off-chain. Casino fees are retained in bankroll equity: losses add S-F/2 and wins subtract W+F/2 to reported betting bankroll. Admission uses the conservative full-fee inequality and reserves W+F. Each bet's even share of F accrues equally to its developer and casino earnings, once per completed bet. The service decides admission from current capital excluding other rounds' reservations and reserves the round's worst case before it reads the round's secret; a wager it has admitted settles. A bet on a channel's own round carries the commission its admission priced; a drawn bet carries its share of what its round is priced at when drawn. Commission never changes the player's signed stake, net win or probability. Insufficient capacity produces a joint higher-sequence rejection checkpoint with unchanged balance and no commission. A channel's own round is declined with its bet, revealed at once and settling nothing; a bet a referee's round cannot take is declined as it is placed, and the round takes others. Reservations are per round, and a referee's round keeps its own until it is drawn or its bets are refunded; they do not fund a whole future hand or stop an owner from withdrawing on-chain.
...
Anyone may add to this capital. An [investment](protocol.md#the-bankroll-fund) is a debit from a channel into the bankroll that mints shares at `equity / totalShares`, where equity is the reported bankroll before round reservations; a redemption burns them at the same price and owes the player their worth, which counts as escrow until collected. Both leave the price of every other share unchanged, so holders gain and lose only what the bankroll does: losses add to equity and wins and developer commission take from it, pro rata. Investors widen what the Kelly rule admits exactly as the owner's own funding does, and the owner's funding and withdrawals buy and sell house shares at the going price. A redemption never takes money a round has reserved.
