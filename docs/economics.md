...
| Symbol    | Meaning                                                                                                                                                            |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| B         | Positive unreserved accounting bankroll at admission, after active player liabilities, finalized claims, unpaid developer commissions, and in-flight reservations. |
...
The service finds the maximum integer `F` satisfying the inequality by bounded binary search. The left side strictly decreases over the allowed fee interval. It rounds `F` down to an even number of wei, so that:
...
One wei of possible split dust stays in the bankroll. Probability rounding can likewise leave a slightly more favorable residual bet. JavaScript BigInt preserves the exact intermediate products.
...
Signed results change the player balance and casino accounting off-chain. Casino fees are retained in bankroll equity: losses add S-F/2 and wins subtract W+F/2 to reported betting bankroll. Admission uses the conservative full-fee inequality and reserves W+F. The full even fee F accrues equally to developer and casino earnings, once per completed bet. The service decides admission from current capital excluding other in-flight reservations and reserves the round's worst case before it reads the round's secret; a wager it has admitted settles, and only the commission gives way if capital has moved by then. It never changes the player's signed stake, net win or probability. Insufficient capacity produces a joint higher-sequence rejection checkpoint with unchanged balance and no commission; the declined round is revealed and settles nothing. Reservations are per round; they do not fund a whole future hand or stop an owner from withdrawing on-chain.
...
Developer cash payments reduce pool cash and their payable at the same confirmed observation. Casino-earned commission is a cumulative reporting counter, not a second payable. Owner withdrawals reduce observed pool cash and thus bankroll equity; no separate house-commission release can be double-counted. Developer cash payouts need operator authorization and use a single-use on-chain withdrawal ID.
...
A native transfer pairs a kind-4 sender debit with a kind-5 recipient credit of the same amount. Pending offers change neither balance. Completion commits both balances together, leaving aggregate active liabilities and accounting bankroll unchanged. Transfers settle on no round and accrue no commission. Principal remains attached to the original deposits; receiving another channel's money does not add collateral or increase the recipient's protected principal.
