...
- the **wallet** ([client/](client/)), which holds the keys, signs exact bets, verifies every result and rejection, and saves the evidence before showing an outcome;
...
- **The casino can withhold completion.** It can decline a casino bet, go offline, or never answer, without a protocol penalty. A rejection is a signed checkpoint that leaves balance and entropy unchanged, and an unanswered operation settles at the latest completed state. Verified results do not prove that every requested casino bet was completed without bias. A declined casino bet's round is revealed with its rejection, and the wallet records at once what the bet would have paid, so selective rejection is visible on the receipt, not prevented.
- **A developer bet trusts its game's developer.** A developer bet is a bet against the game's developer, not the bankroll: its stake goes into the developer's bank at the casino when it is placed, and it is paid what the developer settles, from that bank, with no deadline and no refund. With prizes it is provably fair: it names a round of the developer's whose outcome is fixed before the bet, because the casino names the round and the developer commits its seed to it first, so nobody can change the outcome and neither knows it alone. It is owed what its prizes pay if the developer's casino bet on the round covers it, and its stake back if not, and the wallet checks what it was paid against that and shows any shortfall. Together casino and developer could know the outcome and leave winners uncovered, which each returned bet's receipt shows with what it would have paid. With terms it is paid what the developer signs. What a developer bet is paid is the casino's promise until the wallet collects it: until then it is outside the principal the contract protects. Each receipt says whose word it rests on.
...
- **Playing a game trusts its developer:** for its tables in any game, and with a developer bet for its payment, and for its outcome too when the bet has terms. A developer's bank reserves nothing: whether a developer can pay what its developer bets are owed, and proving it, is between the developer and its players, outside HookedIn.
...
Roulette takes the [developer bets](docs/protocol.md#developer-bets) of many players on one round of its developer's per spin, backs them with one casino bet of its developer's, and ships its page and its server as one Cloudflare Worker. The rest are static pages. The server holds the key of the account the game is published from, the house developer in [catalog.json](catalog.json): a secret, set once from `games/roulette` with `npx wrangler secret put DEVELOPER_KEY`.
