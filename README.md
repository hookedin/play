...
- **Pots trust their referee.** Players share an outcome by entering a pot that the game's referee opens and ends. In a house pot the referee draws the seed and keeps it until it resolves the pot, so nobody knows the outcome while entries are taken; neither referee nor casino can choose it alone, but together they could. A developer's or players' pot ends on the referee's signed word, and a developer's pot pays winners beyond its entries from the developer's bank. A pot nobody ends by its deadline refunds every entry. Each entry's receipt says whose word it rests on.
